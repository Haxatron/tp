...
public class EditCommandParserTest {
    ...
    public void parse_invalidValue_failure() {
        ...
        assertParseFailure(parser, "1" + INVALID_NAME_DESC + INVALID_EMAIL_DESC
                        + VALID_TYPE_STUDENT + VALID_PHONE_AMY,
    ...
    public void parse_allFieldsSpecified_success() {
        ...
        String userInput = targetIndex.getOneBased() + NAME_DESC_AMY + PHONE_DESC_BOB
                + EMAIL_DESC_AMY + TYPE_DESC_STUDENT + TELEGRAM_DESC_AMY + SESSION_DESC_AMY;

        EditPersonDescriptor descriptor = new EditPersonDescriptorBuilder().withName(VALID_NAME_AMY)
                .withPhone(VALID_PHONE_BOB).withEmail(VALID_EMAIL_AMY).withType(VALID_TYPE_STUDENT)
                .withTelegram(VALID_TELEGRAM_AMY).withSession(VALID_SESSION_AMY).build();
    ...
    public void parse_oneFieldSpecified_success() {
        ...
        // type + session
        userInput = targetIndex.getOneBased() + TYPE_DESC_STUDENT + SESSION_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder()
                .withType(VALID_TYPE_STUDENT).withSession(VALID_SESSION_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);

        // session
        userInput = targetIndex.getOneBased() + SESSION_DESC_AMY;
        descriptor = new EditPersonDescriptorBuilder().withSession(VALID_SESSION_AMY).build();
        expectedCommand = new EditCommand(targetIndex, descriptor);
        assertParseSuccess(parser, userInput, expectedCommand);
    }
    ...
    public void parse_multipleRepeatedFields_failure() {
        ...
        userInput = targetIndex.getOneBased() + PHONE_DESC_AMY + TYPE_DESC_STUDENT + EMAIL_DESC_AMY
                + PHONE_DESC_AMY + TYPE_DESC_STUDENT + EMAIL_DESC_AMY + PHONE_DESC_BOB
                + TYPE_DESC_TA + EMAIL_DESC_BOB;

        assertParseFailure(parser, userInput,
                Messages.getErrorMessageForDuplicatePrefixes(PREFIX_PHONE, PREFIX_EMAIL, PREFIX_TYPE));

        // multiple invalid values
        userInput = targetIndex.getOneBased() + INVALID_PHONE_DESC + INVALID_TYPE_DESC + INVALID_EMAIL_DESC
                + INVALID_PHONE_DESC + INVALID_TYPE_DESC + INVALID_EMAIL_DESC;
